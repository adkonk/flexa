...
s_inv.s0 = s_inv.sector_angles(s_inv.x.reshape(-1, 3)) # correct sector angles
...
s3_inv = copy.deepcopy(s3)
s3_inv.x[:(3 * s3_inv.n_cells)] *= 0.5 
s3_inv.x[(3 * s3_inv.n):] *= -1
s3_inv.s0 = s3_inv.sector_angles(s3_inv.x.reshape(-1, 3)) 
...
energies = energies_inv = energies3 = energies3_inv = \
    np.zeros((psis.size, phis.size))
...
for ri in np.arange(inds.shape[0]):
    ...
    s3 = get_sheet(s3, name3, phis[phi_i], psis[psi_i], ell0, k)
    s3_inv = get_sheet(s3_inv, name3_inv, phis[phi_i], psis[psi_i], ell0, k)
    
    energies[psi_i, phi_i] = s.energy(s.x, k)
    energies_inv[psi_i, phi_i] = s_inv.energy(s_inv.x, k)
    energies3[psi_i, phi_i] = s3.energy(s3.x, k)
    energies3_inv[psi_i, phi_i] = s3_inv.energy(s3_inv.x, k)
...
makeplot(name, energies)
makeplot(name_inv, energies_inv)
makeplot(name3, energies3)
makeplot(name3_inv, energies3_inv)