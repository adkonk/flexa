...
class FlexaSheet(object):
    ...
    def grad(self, k, deltas=None):
        ...
        if self.normals == 'lsc':
            ...
            ref = self.cell_normals(r, mode='avg')
            # (n_cells, 3)
            ref = np.array([n for n in ref.values()])

            nvec *= np.sign(rowwise_dot(nvec, ref))[:, np.newaxis]

            normals = {i: nvec[i, :] / np.linalg.norm(nvec[i, :]) for i in \
                range(nvec.shape[0])}
        ...
        if self.normals == 'avg':
            ...
            nvec_norm = np.linalg.norm(nvec, axis=1)[self.collar_edges[:, 0]]
            dn_dr_part1 = np.pad(db_dr / \
                    nvec_norm[np.newaxis, :, np.newaxis, np.newaxis], 
                ((0, 0), (0, 0), (0, 1), (0, 0)))
